...
pub enum DrawCmd {
    ...
    Image { x: f32, y: f32, w: f32, h: f32, path: String },
    ...
    Mermaid { x: f32, y: f32, scale: f32, graph: Arc<mermaid::Graph> },
}
...
impl Ctx {
    fn new(x_base: f32, width: f32, y_start: f32, measure: fn(&str, &str, f32, bool, bool) -> f32) -> Self {
        Self { cmds: Vec::new(), hits: Vec::new(), toolbar_hits: Vec::new(),
               headings: Vec::new(),
               x_base, width, y: y_start, indent: 0.0, measure }
    }

    fn push(&mut self, cmd: DrawCmd) { self.cmds.push(cmd); }

    fn text(&mut self, text: &str, x: f32, y: f32, max_w: f32,
        font: &'static str, size: f32, bold: bool, italic: bool, color: u32, underline: bool)
    {
        if text.is_empty() { return; }
        self.push(DrawCmd::Text {
            x, y, max_w,
            text: text.encode_utf16().collect(),
            font, size, bold, italic, color, underline,
        });
    }
...
pub fn layout(blocks: &[Block], x_base: f32, width: f32, y_start: f32,
              measure: fn(&str, &str, f32, bool, bool) -> f32) -> Layout {
...
fn layout_block(ctx: &mut Ctx, block: &Block, depth: usize) {
    match block {
        ...
        Block::Table { headers, rows, alignments } => layout_table(ctx, headers, rows, alignments),
        Block::Toolbar(items) => layout_toolbar(ctx, items),
        Block::Mermaid { source, graph, error } => layout_mermaid(ctx, source, graph.as_ref(), error.as_deref()),
    }
}
...
fn layout_mermaid(
    ...
    let Some(graph) = graph else {
        if let Some(e) = error {
            layout_paragraph(
                ctx,
                &[Inline::Italic(format!("mermaid error: {e}"))],
            );
        }
...
fn layout_paragraph(ctx: &mut Ctx, inlines: &[Inline]) {
    ...
    if inlines.is_empty() { return; }

    // Fast path: pure plain/bold/italic text with no links or code → single text run.
    let all_same_style = inlines.iter().all(|i| match i {
        Inline::Text(_) | Inline::Bold(_) | Inline::Italic(_) | Inline::BoldItalic(_)
        | Inline::SoftBreak | Inline::HardBreak => true,
        _ => false,
    });
    let first_bold   = matches!(inlines.first(), Some(Inline::Bold(_) | Inline::BoldItalic(_)));
    let first_italic = matches!(inlines.first(), Some(Inline::Italic(_) | Inline::BoldItalic(_)));
    let uniform = all_same_style && inlines.iter().all(|i| match i {
    ...
    if uniform {
        ...
        if text.is_empty() { return; }
        let y = ctx.y;
        ctx.text(&text, x, y, max_w, theme::BODY_FONT, theme::BODY_SIZE,
                 first_bold, first_italic, theme::TEXT, false);
    ...
    for inline in inlines {
        match inline {
            ...
            Inline::Image { .. } => { continue; } // inline images not supported in paragraphs
            _ => {}
        }

        let (raw, bold, italic, underline, color, font, size, href) = match inline {
            Inline::Text(t)       => (t.as_str(), false, false, false, theme::TEXT,    theme::BODY_FONT, theme::BODY_SIZE, None),
            Inline::Bold(t)       => (t.as_str(), true,  false, false, theme::TEXT,    theme::BODY_FONT, theme::BODY_SIZE, None),
            Inline::Italic(t)     => (t.as_str(), false, true,  false, theme::TEXT,    theme::BODY_FONT, theme::BODY_SIZE, None),
            Inline::BoldItalic(t) => (t.as_str(), true,  true,  false, theme::TEXT,    theme::BODY_FONT, theme::BODY_SIZE, None),
            Inline::Code(t)       => (t.as_str(), false, false, false, theme::CODE_FG, theme::CODE_FONT, theme::CODE_SIZE, None),
            Inline::Link { text, href } =>
                (text.as_str(), false, false, true, theme::LINK, theme::BODY_FONT, theme::BODY_SIZE, Some(href.clone())),
        ...
        if matches!(inline, Inline::Code(_)) {
            words.push(Word { text: raw.to_string(), bold, italic, underline, color, font, size, href });
        } else {
    ...
    for word in &words {
        ...
        let word_y = if word.font == theme::CODE_FONT { cur_y + theme::INLINE_CODE_Y } else { cur_y };
        ctx.text(&word.text, cur_x, word_y, avail, word.font, word.size,
                 word.bold, word.italic, word.color, word.underline);

        if let Some(href) = &word.href {
            ctx.hits.push(HitRegion {
                x0: cur_x, y0: cur_y,
                x1: cur_x + w, y1: cur_y + line_h,
                href: href.clone(),
            });
        }
...
fn layout_table(ctx: &mut Ctx, headers: &[String], rows: &[Vec<String>], alignments: &[ColAlign]) {
    ...
    // ── Column width sizing ────────────────────────────────────────────────
    let mut natural: Vec<f32> = headers.iter()
    ...
    for row in rows {
        for (ci, cell) in row.iter().enumerate().take(col_count) {
            let cw = (ctx.measure)(cell, theme::BODY_FONT, theme::BODY_SIZE, false, false) + cell_pad * 2.0;
            if cw > natural[ci] { natural[ci] = cw; }
        }
    }
    ...
    let col_x: Vec<f32> = {
        let mut acc = x;
        col_widths.iter().map(|cw| { let cx = acc; acc += cw; cx }).collect()
    };
    ...
    ctx.push(DrawCmd::FillRect { x, y: ctx.y, w, h: header_h, color: theme::SIDEBAR_BG });
    for (i, header) in headers.iter().enumerate() {
        ...
        ctx.text(header, tx, ctx.y + cell_pad, avail,
                 theme::BODY_FONT, theme::BODY_SIZE, true, false, theme::TEXT_BRIGHT, false);
    }
    ctx.push(DrawCmd::StrokeLine {
        x0: x, y0: ctx.y + header_h, x1: x + w, y1: ctx.y + header_h, color: theme::BORDER,
    });
    ctx.y += header_h;
    ...
    for (ri, row) in rows.iter().enumerate() {
        let row_h = row.iter().enumerate().take(col_count).map(|(ci, cell)| {
        ...
        let row_bg = if ri % 2 == 0 { theme::BG } else { theme::SIDEBAR_BG };
        ctx.push(DrawCmd::FillRect { x, y: ctx.y, w, h: row_h, color: row_bg });
        for (ci, cell) in row.iter().enumerate().take(col_count) {
            ...
            let tw = (ctx.measure)(cell, theme::BODY_FONT, theme::BODY_SIZE, false, false).min(avail);
            let tx = aligned_x(col_left, avail, tw, a);
            ctx.text(cell, tx, ctx.y + cell_pad, avail,
                     theme::BODY_FONT, theme::BODY_SIZE, false, false, theme::TEXT, false);
        }
        ctx.push(DrawCmd::StrokeLine {
            x0: x, y0: ctx.y + row_h, x1: x + w, y1: ctx.y + row_h, color: theme::BORDER,
        });
        ctx.y += row_h;
    }
    ...
    for i in 1..col_count {
        ctx.push(DrawCmd::StrokeLine {
            x0: col_x[i], y0: table_top, x1: col_x[i], y1: ctx.y, color: theme::BORDER,
        });
    }
    ctx.push(DrawCmd::StrokeLine { x0: x,     y0: table_top, x1: x,     y1: ctx.y, color: theme::BORDER });
    ctx.push(DrawCmd::StrokeLine { x0: x + w, y0: table_top, x1: x + w, y1: ctx.y, color: theme::BORDER });
}
...
fn layout_toolbar(ctx: &mut Ctx, items: &[ToolbarItem]) {
    ...
    let total_h = top_pad + icon_h + bot_pad
        + if show_labels { icon_gap + label_sz * theme::LINE_EXTRA } else { 0.0 };

    // Strip background
    ctx.push(DrawCmd::FillRect { x, y: ctx.y, w, h: total_h, color: theme::SIDEBAR_BG });
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        if i + 1 < items.len() {
            ctx.push(DrawCmd::StrokeLine {
                x0: ix + item_w, y0: ctx.y,
                x1: ix + item_w, y1: ctx.y + total_h,
                color: theme::BORDER,
            });
        }
        ...
        ctx.push(DrawCmd::Image {
            x: icon_x, y: icon_y, w: icon_disp, h: icon_disp,
        ...
        if show_labels {
            ...
            ctx.text(&item.label, label_x, label_y, label_w,
                     theme::BODY_FONT, label_sz, false, false, theme::TEXT, false);
        }

        // Hit region for the whole cell
        ctx.toolbar_hits.push(HitRegion {
            x0: ix, y0: ctx.y, x1: ix + item_w, y1: ctx.y + total_h,
            href: item.href.clone(),
        });
    }

    // Top and bottom border lines
    ctx.push(DrawCmd::StrokeLine { x0: x, y0: ctx.y,          x1: x + w, y1: ctx.y,          color: theme::BORDER });
    ctx.push(DrawCmd::StrokeLine { x0: x, y0: ctx.y + total_h, x1: x + w, y1: ctx.y + total_h, color: theme::BORDER });
...
fn estimate_lines(text: &str, max_w: f32, size: f32) -> f32 {
    ...
    (text.chars().count() as f32 / chars_per_line).max(1.0).ceil()
}
