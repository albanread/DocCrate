...
use windows::{
    ...
    Win32::{
        ...
        Graphics::{
            ...
            Dxgi::Common::DXGI_FORMAT_B8G8R8A8_UNORM, Gdi::*,
        },
        System::Com::*,
        UI::{
            HiDpi::GetDpiForWindow, Input::KeyboardAndMouse::*, Shell::ShellExecuteW,
            WindowsAndMessaging::*,
        },
    },
};
...
use crate::{
    docs,
    layout::{self, DrawCmd, Layout},
    mermaid, parser, search, theme,
};

const WM_RELAYOUT: u32 = WM_APP;
const WM_TESTSNAP: u32 = WM_APP + 1;
...
struct App {
    ...
    /// Test automation target: scroll to the rendered block nearest this
    /// source Markdown line after the first layout.
    pending_scroll_line: Option<usize>,
    test_snap_extra_scroll_y: f32,
    test_snap_pending: bool,
}
...
impl App {
    fn new(
        ...
        let current = initial_file
            .and_then(|path| find_initial_file(&files, path))
            .unwrap_or(0);
        Self {
            hwnd,
            target: None,
            ...
            current,
            layout: None,
            scroll_y: if test_snap && test_snap_scroll_to_line.is_none() {
                test_snap_scroll_y.max(0.0)
            } else {
                0.0
            },
            max_scroll: 0.0,
            ...
            pending_scroll_line: if test_snap {
                test_snap_scroll_to_line
            } else {
                None
            },
            test_snap_extra_scroll_y: if test_snap && test_snap_scroll_to_line.is_some() {
                test_snap_scroll_y.max(0.0)
            } else {
                0.0
            },
            test_snap_pending: test_snap,
        }
    }
    ...
    fn screenshot_path(&self) -> PathBuf {
        let docs_dir = self
            .docs_dir
            .canonicalize()
            .unwrap_or_else(|_| self.docs_dir.clone());
        docs_dir
            .parent()
            .unwrap_or(docs_dir.as_path())
            .join("screen.png")
    }

    fn save_screenshot_with_title(&self) {
        let path = self.screenshot_path();
        match unsafe { save_client_png(self.hwnd, &path) } {
            Ok(()) => {
                let name = path
                    .file_name()
                    .and_then(|s| s.to_str())
                    .unwrap_or("screen.png");
                let title = format!("DocCrate \u{2014} saved {}", name);
    ...
    fn toggle_sidebar(&mut self) {
        ...
        unsafe {
            let _ = PostMessageW(Some(self.hwnd), WM_RELAYOUT, WPARAM(0), LPARAM(0));
        }
    }
    ...
    fn relayout(&mut self) {
        ...
        if self.parsed.is_none() {
            ...
            self.parsed = Some(cached.unwrap_or_else(|| {
                parser::parse_loaded(docs::load(&self.files[self.current].path))
            }));
        }
        ...
        self.header_display_h = self
            .header_bitmap
            .as_ref()
            .map(|bmp| unsafe {
                let sz = bmp.GetSize();
                if sz.width > 0.0 {
                    content_w * (sz.height / sz.width)
                } else {
                    0.0
                }
            })
        ...
        if let Some(target_line) = self.pending_scroll_line.take() {
            if let Some(y) = y_for_source_line(&ly.source_lines, target_line) {
                self.scroll_y =
                    (y - theme::V_PAD + self.test_snap_extra_scroll_y).clamp(0.0, self.max_scroll);
            }
        }
    ...
    fn find_rerun(&mut self) {
        ...
        if let Some(blocks) = &self.parsed {
            search::search_doc(self.current, blocks, &q, &mut headings);
        }
        // Search every other warm-cached doc.
        {
            let cache = self.parsed_cache.lock().unwrap();
            for (idx, blocks) in cache.iter() {
                search::search_doc(*idx, blocks, &q, &mut headings);
            }
        }
        headings.truncate(search::MAX_RESULTS);
        self.find_results = headings;
    }
    ...
    fn thumb_rect(&self) -> (f32, f32, f32, f32) {
        ...
        if self.max_scroll <= 0.0 {
            return (tx, 0.0, SCRW, self.height);
        }
    ...
    fn on_scrollbar(&self, x: f32) -> bool {
        x >= self.width - SCRW
    }

    fn hit_sidebar(&self, x: f32, y: f32) -> Option<usize> {
        if self.hidden() || x >= self.sw() {
            return None;
        }
    ...
    fn hit_link(&self, x: f32, y: f32) -> Option<usize> {
        ...
        ly.hits
            .iter()
            .position(|hr| x >= hr.x0 && x <= hr.x1 && dy >= hr.y0 && dy <= hr.y1)
    }
    ...
    fn resize_target(&mut self, w: u32, h: u32) {
        if let Some(t) = &self.target {
            ...
            unsafe {
                let _ = t.Resize(std::ptr::addr_of!(sz));
            }
        }
    }
    ...
    fn paint(&mut self) -> Result<()> {
        ...
        if self.test_snap_pending && self.layout.is_some() {
            self.test_snap_pending = false;
            unsafe {
                let _ = PostMessageW(Some(self.hwnd), WM_TESTSNAP, WPARAM(0), LPARAM(0));
            }
        }
        Ok(())
    }
    ...
    unsafe fn draw_toggle_btn(&self, t: &ID2D1HwndRenderTarget) -> Result<()> {
        let (bx, by, bw, bh) = self.toggle_btn_rect();
        let bg_hex = if self.hover_toggle {
            theme::SIDEBAR_SEL
        } else {
            theme::SIDEBAR_BG
        };
        let fg_hex = if self.hover_toggle {
            theme::TEXT_BRIGHT
        } else {
            theme::TEXT_DIM
        };
        ...
        // Chevron glyph
        let ch: Vec<u16> = if self.hidden() { "›" } else { "‹" }
            .encode_utf16()
            .collect();
        ...
        for (i, entry) in self.sidebar.iter().enumerate() {
            ...
            match entry {
                docs::SidebarEntry::File { file_idx, depth } => {
                    ...
                    let display: Vec<u16> =
                        pretty(&self.files[*file_idx].name).encode_utf16().collect();
                    let r2 = D2D_RECT_F {
                        left: indent,
                        top: y + 5.0,
                        right: sw - 10.0,
                        bottom: y + theme::SIDEBAR_ITEM_H,
                    };
                    t.DrawText(
                        &display,
                        &fmt2,
    ...
    unsafe fn draw_content(&self, t: &ID2D1HwndRenderTarget) -> Result<()> {
        ...
        if let Some(ti) = self.hover_toolbar {
            let hr_data = self
                .layout
                .as_ref()
                .and_then(|ly| ly.toolbar_hits.get(ti))
                .cloned();
            if let Some(hr) = hr_data {
                let ry0 = hr.y0 + oy;
                let ry1 = hr.y1 + oy;
    ...
    unsafe fn draw_find_panel(&self, t: &ID2D1HwndRenderTarget) -> Result<()> {
        ...
        if self.find_query.is_empty() {
            let br_dim = self.brush(t, theme::TEXT_DIM)?;
            let placeholder: Vec<u16> = "Search headings\u{2026}".encode_utf16().collect();
            t.DrawText(
                &placeholder,
                &fmt_in,
        ...
        for i in 0..visible {
            ...
            let lab_rect = D2D_RECT_F {
                left: rx + 34.0,
                top: ry,
                right: rx + rw - 12.0 - file_w,
                bottom: ry + rh,
            };
            t.DrawText(
                &lab_utf,
                &fmt_l,
                std::ptr::addr_of!(lab_rect),
                &br_l,
                D2D1_DRAW_TEXT_OPTIONS_CLIP,
                DWRITE_MEASURING_MODE_NATURAL,
            );
...
pub fn run_with_options(
    ...
    unsafe {
        ...
        // Pre-warm small docs on worker threads. Large docs are parsed on
        // demand so startup never eagerly chews through huge files.
        spawn_parse_workers(cache_handle, file_paths);
...
fn find_initial_file(files: &[docs::DocFile], initial_file: &Path) -> Option<usize> {
    let wanted = initial_file.canonicalize().ok()?;
...
fn y_for_source_line(source_lines: &[(usize, f32)], target_line: usize) -> Option<f32> {
    let mut best = None;
    for &(line, y) in source_lines {
        if line <= target_line {
            best = Some(y);
        } else {
            return best.or(Some(y));
        }
    }
    best
}
...
fn spawn_parse_workers(
    cache_handle: Arc<Mutex<std::collections::HashMap<usize, Vec<crate::parser::Block>>>>,
    file_paths: Vec<PathBuf>,
) {
    let mut task_vec: Vec<(usize, PathBuf)> = file_paths.into_iter().enumerate().skip(1).collect();
    task_vec.sort_by_key(|(_, path)| {
        let len = path.metadata().map(|m| m.len()).unwrap_or(0);
        (len >= docs::LARGE_DOC_BYTES, len)
    });
    let tasks: std::collections::VecDeque<(usize, PathBuf)> = task_vec.into();
    if tasks.is_empty() {
        return;
    }
    ...
    for _ in 0..worker_count {
        ...
        std::thread::spawn(move || loop {
            let Some((idx, path)) = tasks.lock().unwrap().pop_front() else {
                break;
            };

            if cache_handle.lock().unwrap().contains_key(&idx) {
                continue;
            }

            let blocks = parser::parse_loaded(docs::load(&path));
            cache_handle.lock().unwrap().entry(idx).or_insert(blocks);
        });
    }
}
    ...
    match msg {
        ...
        WM_MOUSEMOVE => {
            ...
            if a.dragging_scroll {
                ...
                if track > 0.0 {
                    a.scroll_y =
                        (a.drag_start_scroll + dy / track * a.max_scroll).clamp(0.0, a.max_scroll);
                }
                let _ = InvalidateRect(Some(hwnd), None, false);
                return LRESULT(0);
            }
            ...
            // ── dragging divider ───────────────────────────────────────────
            if a.dragging_div {
                let new_w = (a.div_start_w + (x - a.div_start_x)).clamp(SB_MIN, a.width * 0.65);
                a.sidebar_w = new_w;
                let _ = InvalidateRect(Some(hwnd), None, false);
                return LRESULT(0);
            }
            ...
            let in_find_input =
                a.find_open && a.point_in_find_panel(x, y) && a.hover_find_row.is_none();
            let cursor = if a.hover_div {
                LoadCursorW(None, IDC_SIZEWE).unwrap()
            } else if in_find_input {
                LoadCursorW(None, IDC_IBEAM).unwrap()
            } else if a.hover_find_row.is_some()
                || a.hover_link.is_some()
                || a.hover_toolbar.is_some()
                || a.hover_toggle
                || a.hover_tab
                || sb_is_file
            {
            ...
            if a.hover_div != prev_div
                || a.hover_toggle != prev_toggle
                || a.hover_tab != prev_tab
                || a.hover_sidebar != prev_sb
                || a.hover_link != prev_lk
                || a.hover_toolbar != prev_tb
        ...
        WM_LBUTTONUP => {
            if !ptr.is_null() {
                let a = &mut *ptr;
                if a.dragging_scroll {
                    a.dragging_scroll = false;
                }
                if a.dragging_div {
                    a.dragging_div = false;
                    a.layout = None;
                    let _ = PostMessageW(Some(hwnd), WM_RELAYOUT, WPARAM(0), LPARAM(0));
                }
                let _ = ReleaseCapture();
        ...
        WM_KEYDOWN => {
            ...
            if a.find_open {
                match vk {
                    ...
                    VK_UP => {
                        a.find_selected = a.find_selected.saturating_sub(1);
                    }
                    VK_RETURN => {
                        a.find_activate();
                    }
...
fn capture_error(message: &str) -> Error {
    Error::new(HRESULT(0x8000_4005u32 as i32), message)
}

unsafe fn save_client_png(hwnd: HWND, path: &Path) -> Result<()> {
    let mut rc = RECT::default();
    GetClientRect(hwnd, &mut rc)?;
    let width = rc.right - rc.left;
    let height = rc.bottom - rc.top;
    if width <= 0 || height <= 0 {
        return Err(capture_error("window client area is empty"));
    }
    ...
    if src.hdc.is_invalid() {
        return Err(Error::from_thread());
    }

    let mem = MemDc(CreateCompatibleDC(Some(src.hdc)));
    if mem.0.is_invalid() {
        return Err(Error::from_thread());
    }

    let bitmap = GdiBitmap(CreateCompatibleBitmap(src.hdc, width, height));
    ...
    let _selected = SelectGuard { hdc: mem.0, old };

    BitBlt(mem.0, 0, 0, width, height, Some(src.hdc), 0, 0, SRCCOPY)?;
    save_hbitmap_png(bitmap.0, path, width as u32, height as u32)
}

unsafe fn save_hbitmap_png(hbitmap: HBITMAP, path: &Path, width: u32, height: u32) -> Result<()> {
...

    {
        ...
        let frame = frame.ok_or_else(|| capture_error("png encoder did not create a frame"))?;
...
fn lo_word(lp: LPARAM) -> u32 {
    (lp.0 & 0xFFFF) as u32
}
fn hi_word(lp: LPARAM) -> u32 {
    ((lp.0 >> 16) & 0xFFFF) as u32
}
